...
CREATE TABLE tbl_book (
    ...
    CONSTRAINT fk_publisher_name FOREIGN KEY (book_PublisherName) REFERENCES tbl_publisher(publisher_PublisherName) ON UPDATE CASCADE ON DELETE CASCADE
);
...
SELECT COUNT(book_copies_No_Of_Copies) FROM tbl_book_copies where book_copies_BookID = 20 AND book_copies_BranchID = 1;

DELIMITER //
CREATE PROCEDURE bookCopiesAtAllSharpstown (IN bookTitle CHAR(70), IN branchName CHAR(70))
BEGIN
...
-- DROP PROCEDURE bookCopiesAtAllSharpstown;    
CALL bookCopiesAtAllSharpstown('The Lost Tribe', 'Sharpstown');

/* #2- How many copies of the book titled "The Lost Tribe" are owned by each library branch? */

DELIMITER //
CREATE PROCEDURE bookCopiesAtAllBranches (IN bookTitle char(70) )
BEGIN
...
DELIMITER //
CREATE PROCEDURE LoanersInfo (IN DueDate date  , IN LibraryBranchName varchar(50))
begin
SET DueDate = curdate();
...
call LoanersInfo(NULL, 'Sharpstown');
...
delimiter //
CREATE PROCedure BookbyAuthorandBranch (IN BranchName char(50) ,  IN AuthorName char(50) )
begin
...
BEFORE DELETE, ALTER TABLE, DROP TABLE
ON librarymanagement
FOR EACH ROW
BEGIN
  -- Print a message to the user
  SELECT 'You cannot create, drop, or alter tables on this database.';
END //
DELIMITER ;
